...
function col(name, text) {
  return IS_TTY ? `${C[name]}${text}${C.reset}` : text;
}
function bold(text) {
  return IS_TTY ? `${C.bold}${text}${C.reset}` : text;
}
function dim(text) {
  return IS_TTY ? `${C.dim}${text}${C.reset}` : text;
}
...
function banner() {
  ...
  console.log(
    col("cyan", "║") +
      bold("  Guitar Audio Correction Tool  v1.0  ") +
      col("cyan", "║"),
  );
  console.log(col("cyan", "╚══════════════════════════════════════╝"));
  console.log();
...
async function findPython() {
  for (const bin of ["python3", "python", "py"]) {
    const ok = await new Promise((res) => {
      const p = spawn(bin, ["-c", "import numpy"], { stdio: "ignore" });
...
async function runPython(pyArgs, onProgress) {
  const python = await findPython();

  const scriptPath = path.join(__dirname, "audio_processor.py");
  if (!fs.existsSync(scriptPath))
    throw new Error(`audio_processor.py not found at ${scriptPath}`);

  return new Promise((resolve, reject) => {
    const proc = spawn(python, [scriptPath, ...pyArgs], {
...
async function cmdAnalyze(inputFile, opts) {
  ...
  if (s.mode) {
    const modeLabel = s.mode === "chord" ? col("cyan", "chord (chromagram)") : col("green", "mono (pYIN)");
    console.log(`  Mode:               ${modeLabel}`);
  }
  console.log(`  Segments detected:  ${bold(s.total_notes)}`);
  console.log(
    `  Flagged:            ${s.flagged_notes > 0 ? col("yellow", s.flagged_notes) : col("green", s.flagged_notes)}`,
  );
  console.log(
    `    Buzz issues:      ${s.buzz_notes > 0 ? col("red", s.buzz_notes) : col("green", s.buzz_notes)}`,
  );
  if (s.mode !== "chord") {
    console.log(
      `    Off-pitch:        ${s.off_pitch_notes > 0 ? col("yellow", s.off_pitch_notes) : col("green", s.off_pitch_notes)}`,
    );
  ...
  const isChord = analysis.mode === "chord";

  if (isChord) {
    // In chord mode confidence is chroma energy, not pYIN probability — don't
    // use low-confidence as a flag criterion.  Only real buzz issues matter.
    const buzzed = analysis.notes.filter((n) => n.has_buzz);
    if (buzzed.length > 0) {
      console.log(col("red", `⚠  Buzz detected in ${buzzed.length} segment(s):`));
      buzzed.forEach((n) => console.log(fmtNoteRow(n)));
    } else {
      console.log(col("green", "✓ No buzz detected"));
    }
    console.log();
    console.log(dim(`${analysis.notes.length} chord segments mapped (dominant pitch class shown)`));
    console.log();
  } else {
    const flagged = analysis.notes.filter(
      (n) => n.has_buzz || n.off_pitch || n.confidence < 0.5,
    );
    if (flagged.length > 0) {
  ...
  console.log(
    `  ${col("cyan", `node cli.js correct ${inputFile} --fixes ${outputJson}`)}`,
  );
  console.log(
    `  ${col("cyan", `node cli.js preview ${inputFile} --analysis ${outputJson}`)}`,
  );
  console.log();
}
...
async function cmdCorrect(inputFile, opts) {
  ...
  const outputMidi = midiFlag
    ? midiFlag === true
      ? stripExt(inputFile) + ".mid"
      : midiFlag
    : null;
  const aggr = parseFloat(opts.aggressiveness || "0.8");
  const smoothVib =
    opts["smooth-vibrato"] === true || opts["smooth-vibrato"] === "true";
  ...
  console.log(
    `${bold("Aggressiveness:")}   ${col("cyan", (aggr * 100).toFixed(0) + "%")}`,
  );
  if (smoothVib)
    console.log(
      `${bold("Vibrato:")}          ${col("yellow", "smoothing on (OLA)")}`,
    );
  console.log();
  ...
  console.log(
    `  Notes corrected:    ${s.notes_corrected > 0 ? col("yellow", s.notes_corrected) : col("green", s.notes_corrected)}`,
  );
  ...
  if (result.report && fs.existsSync(result.report)) {
    ...
    if (rpt.corrections.length > 0) {
      ...
      rpt.corrections.forEach((c) => {
        const noteStr =
          c.original_note !== c.corrected_note
            ? `${dim(c.original_note)} → ${col("green", c.corrected_note)}`
            : col("white", c.original_note);
        console.log(
          `  ${dim(c.timestamp.toFixed(2) + "s")}  ${padVisual(noteStr, 18)}  ${dim(c.actions.join("  "))}`,
        );
      });
      console.log();
    }
  ...
  console.log(
    `  ${col("green", "✓")} Corrected audio:  ${col("cyan", result.output_wav)}`,
  );
  if (result.output_midi)
    console.log(
      `  ${col("green", "✓")} MIDI file:        ${col("cyan", result.output_midi)}`,
    );
  console.log(
    `  ${col("green", "✓")} Report JSON:      ${col("cyan", result.report)}`,
  );
  console.log(dim("────────────────────────────────────────────"));
  console.log();
}
...
async function cmdReview(analysisFile, opts) {
  ...
  if (!process.stdin.isTTY)
    die("review requires an interactive terminal (cannot pipe)");
  ...
  for (const n of flagged) {
    if (n.pitch_correct_approved === undefined)
      n.pitch_correct_approved = n.off_pitch;
    if (n.buzz_remove_approved === undefined)
      n.buzz_remove_approved = n.has_buzz;
  }
  ...
  function render() {
    ...
    const ln = (s = "") =>
      process.stdout.write(
        col("cyan", "│") + " " + padVisual(s, W) + " " + col("cyan", "│\n"),
      );
    ...
    ln(
      dim(
        `Flagged note ${idx + 1} of ${flagged.length}  ·  ${path.basename(analysisFile)}`,
      ),
    );
    console.log(sep);

    ln(
      `Note: ${bold(n.original_note)}   at ${n.timestamp.toFixed(2)}s   dur: ${n.duration.toFixed(2)}s`,
    );
    ln(
      `Confidence: ${confColor(n.confidence)}   Freq: ${n.original_freq_hz.toFixed(1)} Hz`,
    );
    ln();
    ...
    if (n.off_pitch) {
      ...
      ln(
        col(
          "yellow",
          `⚠ Off-pitch: ${sign}${n.pitch_deviation_cents.toFixed(0)}¢ from nearest semitone`,
        ),
      );
    }
    if (n.has_buzz)
      ln(col("red", `⚠ Buzz detected  (score: ${n.buzz_score.toFixed(3)})`));
    if (n.has_vibrato)
      ln(
        col("blue", `≈ Vibrato: ±${n.vibrato_semitones.toFixed(2)} semitones`),
      );
    if (!n.off_pitch && !n.has_buzz && !n.has_vibrato)
      ln(col("yellow", "⚠ Low confidence"));

    console.log(sep);

    const pState = n.pitch_correct_approved
      ? col("green", "● ON ")
      : col("red", "○ OFF");
    const bState = n.buzz_remove_approved
      ? col("green", "● ON ")
      : col("red", "○ OFF");
    ...
    console.log(sep);
    ln(
      `${dim("← →")} navigate   ${dim("A")} approve rest   ${dim("S")} skip rest   ${dim("Q")} save & quit`,
    );
    console.log(col("cyan", "└" + "─".repeat(W + 2) + "┘"));
  ...
  return new Promise((resolve) => {
    ...
    const cleanup = (save) => {
      ...
      if (save) {
        ...
        const nOn = flagged.filter(
          (n) => n.pitch_correct_approved || n.buzz_remove_approved,
        ).length;
        console.log(
          col(
            "green",
            `✓ Saved: ${nOn}/${flagged.length} notes queued for correction`,
          ),
        );
        ...
        console.log(
          `  ${col("cyan", `node cli.js correct ${analysis.input_file} --fixes ${analysisFile}`)}`,
        );
      } else {
    ...
    process.stdin.on("keypress", (str, key) => {
      ...
      switch (true) {
        case key.name === "right":
          idx = Math.min(idx + 1, flagged.length - 1);
          break;
        case key.name === "left":
          idx = Math.max(idx - 1, 0);
          break;
        case str === "p" || str === "P":
          n.pitch_correct_approved = !n.pitch_correct_approved;
          break;
        case str === "b" || str === "B":
          n.buzz_remove_approved = !n.buzz_remove_approved;
          break;
        case str === "a" || str === "A":
...
async function cmdPreview(inputFile, opts) {
  ...
  const analysisFile =
    typeof opts.analysis === "string"
      ? opts.analysis
      : typeof opts.fixes === "string"
        ? opts.fixes
        : null;
  const outputPng = opts.output || stripExt(inputFile) + "_spectrogram.png";

  if (compareFile && !fs.existsSync(compareFile))
    die(`--compare file not found: ${compareFile}`);
  if (analysisFile && !fs.existsSync(analysisFile))
    die(`--analysis file not found: ${analysisFile}`);

  console.log(`${bold("Input:")}    ${col("cyan", inputFile)}`);
  if (compareFile)
    console.log(`${bold("Compare:")}  ${col("cyan", compareFile)}`);
  if (analysisFile)
    console.log(`${bold("Analysis:")} ${col("cyan", analysisFile)}`);
...
function openFile(filePath) {
  // On Windows, use 'cmd /c start' with a blank title so paths with spaces work.
...
// ---------------------------------------------------------------------------
// fix – one-shot analyze + correct (the "1 take" command)
// ---------------------------------------------------------------------------
async function cmdFix(inputFile, opts) {
  banner();
  requireFile(inputFile);

  const outputWav = opts.output || stripExt(inputFile) + "_corrected.wav";
  const outputAnalysis = opts["output-analysis"] || null;
  const midiFlag = opts["output-midi"];
  const outputMidi = midiFlag
    ? midiFlag === true
      ? stripExt(inputFile) + ".mid"
  ...
  if (outputMidi)
    console.log(`${bold("Output MIDI:")}      ${col("cyan", outputMidi)}`);
  if (outputAnalysis)
    console.log(`${bold("Analysis JSON:")}    ${col("cyan", outputAnalysis)}`);
  console.log(
    `${bold("Aggressiveness:")}   ${col("cyan", (aggr * 100).toFixed(0) + "%")}`,
  );
  if (smoothVib)
  ...
  console.log();
  console.log(col("blue", "▶ Analysing & correcting in one pass…"));
  console.log();

  const { onProgress, flush } = makeProgressHandler();

  // Python 'fix' emits two JSON results (analyze then correct) on stdout, newline-separated
  // We capture both and surface the correction result.
  const python = await findPython();
  const scriptPath = path.join(__dirname, "audio_processor.py");
  if (!fs.existsSync(scriptPath))
    die(`audio_processor.py not found at ${scriptPath}`);
  ...
  const pyArgs = [
    ...
    String(aggr),
    ...(outputMidi ? ["--output-midi", outputMidi] : []),
    ...(outputAnalysis ? ["--output-analysis", outputAnalysis] : []),
    ...(smoothVib ? ["--smooth-vibrato"] : []),
    ...(opts.sr ? ["--sr", String(opts.sr)] : []),
    ...(opts.hop ? ["--hop", String(opts.hop)] : []),
    ...(opts.mode ? ["--mode", opts.mode] : []),
  ...
  // results[0] = analyze result, results[1] = correct result
  const analyzeResult = results[0];
  const correctResult = results[1] || results[0];

  exitOnFailure(analyzeResult, "Analysis");
  exitOnFailure(correctResult, "Correction");
  ...
  console.log(`  Duration:           ${bold(as.duration + "s")}`);
  console.log(`  Tempo:              ${bold(as.tempo_bpm + " BPM")}`);
  console.log(`  Notes detected:     ${bold(as.total_notes)}`);
  console.log(
    `  Flagged:            ${as.flagged_notes > 0 ? col("yellow", as.flagged_notes) : col("green", as.flagged_notes)}`,
  );
  console.log(
    `    Buzz issues:      ${as.buzz_notes > 0 ? col("red", as.buzz_notes) : col("green", as.buzz_notes)}`,
  );
  console.log(
    `    Off-pitch:        ${as.off_pitch_notes > 0 ? col("yellow", as.off_pitch_notes) : col("green", as.off_pitch_notes)}`,
  );
  console.log(`  Avg confidence:     ${confColor(as.average_confidence)}`);
  console.log(bold("─── Correction ───────────────────────────"));
  console.log(
    `  Notes corrected:    ${cs.notes_corrected > 0 ? col("yellow", cs.notes_corrected) : col("green", cs.notes_corrected)}`,
  );
  console.log(dim("──────────────────────────────────────────"));
  console.log();
  ...
  console.log(bold("─── Output files ──────────────────────────"));
  console.log(
    `  ${col("green", "✓")} Corrected audio:  ${col("cyan", correctResult.output_wav)}`,
  );
  if (correctResult.output_midi)
    console.log(
      `  ${col("green", "✓")} MIDI file:        ${col("cyan", correctResult.output_midi)}`,
    );
  if (outputAnalysis)
    console.log(
      `  ${col("green", "✓")} Analysis JSON:    ${col("cyan", outputAnalysis)}`,
    );
  console.log(dim("────────────────────────────────────────────"));
  console.log();
}
...
`;
  ...
  const lines = await new Promise((res, rej) => {
    const p = spawn(python, ["-c", pyCheck], {
      stdio: ["ignore", "pipe", "pipe"],
    });
    let out = "";
    p.stdout.on("data", (d) => {
  ...
  for (const line of lines) {
    ...
    if (status === "OK") console.log(`  ${col("green", "✓")} ${mod}`);
...
function printHelp() {
  ...
  console.log(
    `  ${col("cyan", "node cli.js fix")}       ${bold("<input.wav>")}  [options]          ${col("green", "← one-shot: analyze + correct")}`,
  );
  console.log(
    `  ${col("cyan", "node cli.js analyze")}   <input.wav>  [options]`,
  );
  console.log(`  ${col("cyan", "node cli.js review")}    <analysis.json>`);
  console.log(
    `  ${col("cyan", "node cli.js correct")}   <input.wav>  --fixes <analysis.json>  [options]`,
  );
  console.log(
    `  ${col("cyan", "node cli.js preview")}   <input.wav>  [options]`,
  );
  console.log(`  ${col("cyan", "node cli.js check-deps")}`);
  console.log();
  console.log(bold("fix options  (analyze + correct in one command):"));
  console.log(
    `  ${dim("--output <path>")}             Output WAV           (default: <input>_corrected.wav)`,
  );
  console.log(
    `  ${dim("--output-midi [<path>]")}      Export MIDI          (auto-named if no path)`,
  );
  console.log(
    `  ${dim("--output-analysis <path>")}    Save analysis JSON   (discarded by default)`,
  );
  console.log(
    `  ${dim("--aggressiveness <0-1>")}      Correction strength  (default: 0.8)`,
  );
  console.log(
    `  ${dim("--smooth-vibrato")}             Remove vibrato via OLA pitch correction`,
  );
  console.log();
  console.log(bold("analyze options:"));
  console.log(
    `  ${dim("--output <path>")}        JSON output  (default: <input>_analysis.json)`,
  );
  console.log(
    `  ${dim("--sr     <rate>")}         Sample rate  (default: 22050)`,
  );
  console.log();
  console.log(bold("review keys:"));
  console.log(
    `  ${dim("← →")}  navigate notes   ${dim("P")}  toggle pitch correction`,
  );
  console.log(
    `  ${dim("B")}    toggle buzz removal   ${dim("A")}  approve rest   ${dim("S")}  skip rest   ${dim("Q")}  save & quit`,
  );
  console.log();
  ...
  console.log(
    `  ${dim("--output-midi [<path>]")}   Export MIDI    (auto-named if no path given)`,
  );
  console.log(
    `  ${dim("--aggressiveness <0-1>")}   Correction strength  (default: 0.8)`,
  );
  ...
  console.log(
    `  ${dim("--analysis <path>")}        Overlay note markers from analysis JSON`,
  );
  console.log(
    `  ${dim("--compare  <path>")}        Side-by-side with corrected WAV`,
  );
  console.log(
    `  ${dim("--output   <path>")}        PNG output  (default: <input>_spectrogram.png)`,
  );
  console.log(`  ${dim("--no-open")}                Don't launch image viewer`);
  console.log();
  console.log(bold("Examples:"));
  console.log(
    `  ${col("green", "node cli.js fix guitar.wav --smooth-vibrato --output-midi")}   ${dim("# ← do everything in one shot")}`,
  );
  console.log(`  ${dim("node cli.js analyze  guitar.wav")}`);
  console.log(`  ${dim("node cli.js review   guitar_analysis.json")}`);
  console.log(
    `  ${dim("node cli.js correct  guitar.wav --fixes guitar_analysis.json --output-midi --smooth-vibrato")}`,
  );
  console.log(
    `  ${dim("node cli.js preview  guitar.wav --analysis guitar_analysis.json --compare guitar_corrected.wav")}`,
  );
  console.log();
}
...
async function main() {
  ...
  try {
    switch (cmd) {
      case "fix":
        if (!pos[0]) die("Input file required.\n  node cli.js fix <input.wav>");
        await cmdFix(pos[0], opts);
        break;
      ...
      case "review":
        if (!pos[0])
          die("Analysis JSON required.\n  node cli.js review <analysis.json>");
        await cmdReview(pos[0], opts);
        break;
...
// ---------------------------------------------------------------------------
// Exports (only when required as a module, not when run directly)
// ---------------------------------------------------------------------------
